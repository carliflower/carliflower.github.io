  ...
  function MembersService($log, $firebaseArray, DataService) {
    ...
    function tallyPickPoints(houseguests, member) {
      ...
      for (var h = 0; h < houseguests.length; h++) {
        ...
        for (var m = 0; m < picks.length; m++) {
          ...
          if (pick == h) {
            tally +=
              houseguests[h].hoh + houseguests[h].pov + houseguests[h].weeks;
          }
        }
      }
    ...
    function gatherAllMemberPicks(members) {
      ...
      for (var m = 0; m < members.length; m++) {
        ...
        var memberPicksAsSortedString = this.setMemberPicksAsString(
          memberPicksArray
        );
    ...
    function generateRandomPick(members, houseguests) {
      ...
      while (pickSetIsUsed) {
        //generate unique random index #s
        randomSample = _.sample(
          randomizedHouseguestsOrder,
          this.DataService.teamSize
        );
