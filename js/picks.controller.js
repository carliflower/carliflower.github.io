...
;(function() {
  ...
  PicksCtrl.$inject = ["$log", "$scope", "MembersService", "HouseguestsService"]
  ...
  function PicksCtrl($log, $scope, MembersService, HouseguestsService) {
    ...
    function onPickCodeChange() {
      vm.check = vm.MembersService.findMemberByPickCode(vm.members, vm.pickCode)
      ...
      if (vm.check.isValidCode && vm.check.member && !vm.check.member.paid) {
        ...
        //   vm.codeMessage = "You must pay to enable your pick code."
        // }
      }
    }
    ...
    function savePicksCopy() {
      ...
      if (pickCount < vm.maxPicks) {
        s =
          vm.maxPicks - pickCount + " picks left before you can save your team."
      }
      return s
    }
